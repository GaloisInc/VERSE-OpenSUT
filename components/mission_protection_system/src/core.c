...
#ifndef CN_ENV
// TODO there is UB in these files, but they're autogenerated and editing them seems fruitless
...
/*$
  ...
  // TODO we need these here to call update_ui_instr but they can't be here or
  // core_step can't call this, and they can't be in core_step because of scope
    //accesses sensor_warning;
    //accesses sensor_ok;
    requires take uii = Owned<struct ui_values>(ui);
    ensures take uio = Owned<struct ui_values>(ui);
...
int set_display_line(struct ui_values *ui, uint8_t line_number, char *display, uint32_t size) {
  // horrible
  /*$ focus RW<char[66]>, (u64) line_number; $*/
  uint8_t *dis = (uint8_t*)&ui->display[line_number];
  /*$ apply TransmuteArrayRW_char_u8(dis, LINELENGTH()+1u64); $*/
  /*$ apply SplitAt_Owned_u8(dis, LINELENGTH()+1u64, 0u64, LINELENGTH()); $*/
  #if 0
  memset(ui->display[line_number], ' ', LINELENGTH);
  strncpy(ui->display[line_number], (const char*)display, LINELENGTH);
  #else
  memset((char*)dis, ' ', LINELENGTH);
  /*$ apply UnTransmuteArrayRW_char_u8(dis, LINELENGTH()); $*/
  if (size < LINELENGTH) {
    /*$ apply SplitAtRW_char(dis, LINELENGTH(), 0u64, (u64)size); $*/
    ///*$ apply SplitAtRW_char(display, (u64)size, 0u64, (u64)size); $*/
    strncpy((char*)dis, (const char*)display, size);
    /*$ apply UnSplitAtRW_char(dis, LINELENGTH(), 0u64, (u64)size); $*/
    ///*$ apply UnSplitAtRW_char(display, (u64)size, 0u64, (u64)size); $*/
  } else {
    /*$ apply SplitAtRW_char(dis, LINELENGTH(), 0u64, LINELENGTH()); $*/
    /*$ apply SplitAtRW_char(display, (u64)size, 0u64, LINELENGTH()); $*/
    strncpy((char*)dis, (const char*)display, LINELENGTH);
    /*$ apply UnSplitAtRW_char(dis, LINELENGTH(), 0u64, LINELENGTH()); $*/
    /*$ apply UnSplitAtRW_char(display, (u64)size, 0u64, LINELENGTH()); $*/
  }
  // TODO need to split display at LINELENGTH
  /*$ apply TransmuteArrayRW_char_u8(dis, LINELENGTH()); $*/
  #endif
  /*$ apply UnSplitAt_Owned_u8(dis, LINELENGTH()+1u64, 0u64, LINELENGTH()); $*/
  /*$ apply UnTransmuteArrayRW_char_u8(dis, LINELENGTH()+1u64); $*/
  return 0;
}
...
#ifdef CN_ENV
  /*$ extract RW<uint8_t>, 0u64; $*/
  c->test.test_instrumentation[0] = 0;
  /*$ extract RW<uint8_t>, 1u64; $*/
  c->test.test_instrumentation[1] = 0;
  c->test.test_device = 0;
  c->test.test_actuation_unit = 0;
#endif
